...
abstract class DataBase {
    ...
    //Получаем записи
    abstract public  function getRecord($sql);
    //Получаем запись
    abstract public function getRecords($sql);
    //Добавляем запись
    abstract function addRecord($data=array(),$debug=false);
    //Удаляем запись(-и)
    abstract function deleteRecords($table, $where=NULL,$debug=false);
    //Обновляем запись(-и)
    abstract function setRecords($table,$select,$where,$debug=false);
     ...
     *
     * */
    protected function prepareSelectSQL($sql)
    {
	...
	protected function checkWhere($query)
	{
		...
		{
			$change = '=?#';
			$out = preg_replace('/\=.+#/Uis', $change, $query); // перебиваем все на знаки равно и вопроса
			$out = str_replace('#', '', $out);
			//echo $out.'<br />';
			
			// это для удобства, может тебе понадобится
			preg_match_all('/#(.*)#/Uis', $query, $array); // регулярка на полный шаблон от @@ до @@;
