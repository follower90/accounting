<?php
	...
	class IndexController extends BaseController {
		...
		function indexAction()
		{
			...
			//year stats
			$vars['statSpentYear'] = array();
			$vars['statGotYear'] = array();

			foreach($statGotYear as $row) {
				$vars['statGotYear'][$row['year']] = $row['sum'];
			}
			foreach($statSpentYear as $row) {
				$vars['statSpentYear'][$row['year']] = $row['sum'];
			}
			...
			if(isset($_POST['AddNew']))
			{
				...
				if($vars['error']=='')
				{
					...
					$entry = new Entry();
					$entry->setValues(array('name', 'user_id', 'category_id', 'date', 'sum'), 
