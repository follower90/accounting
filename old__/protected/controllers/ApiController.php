...
class ApiController extends BaseController {
	...
	private function showEntries($user_id)
	{
		...
		$types = $this->db->rows("SELECT * FROM `categories`");
	...
	private function newEntry() 
	{
		...
		$this->db->query("INSERT INTO `entries` SET `name`=?, `sum`=?, `date`=?, `category_id`=?",
	...
	private function updateEntry() 
	{
		...
		$this->db->query("UPDATE `entries` SET `name`=?, `sum`=?, `date`=?, `category_id`=? WHERE `id`=?",
	...
	private function deleteEntry($id) {
		...
		$this->db->query("DELETE FROM `entries` WHERE `id`=?", array($id));
