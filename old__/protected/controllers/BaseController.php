...
class BaseController{
	...
	public function Render($param = array())
	{
		...
		$data['meta']['title'] = 'Бухгалтерия 0.5.5 beta';

        $data['styles'] = $view->Load(array('style.css', 'bootstrap.min.css', 'datepicker.css', 'select.css'), 'styles');
    	...
    	$data['types'] = $this->showTypes();
        if((int)$user_id==0) return ($view->Render("login.phtml", $data));
   		else return ($view->Render("index.phtml", $data));
	}
	...
	{
		$types = $this->db->rows("SELECT c.* FROM `categories` c
